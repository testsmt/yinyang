...
def typecheck_fp_unary_bool_rt(expr, ctxt):
    ...
    """
    arg = expr.subterms[0]
    if not isinstance(typecheck_expr(arg,ctxt),FP_TYPE):
...
def typecheck_let_expression(expr,ctxt):
    ...
    for i in range(l):
        ...
        t = typecheck_expr(expr.let_terms[i])
